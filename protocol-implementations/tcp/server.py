...
#
# When we pass data to sendall(), the kernel wraps it in protocol headers
# before putting it on the wire:
#
#   [Ethernet 14B] [IP 20B] [TCP 20B] [our data]
#
# Our code only ever touches "our data". Everything to the left is added
# by the kernel on send and stripped on receive.
#
# By specifying SOCK_STREAM (TCP), the kernel handles:
#   - Three-way handshake (SYN, SYN-ACK, ACK) — completed by the time
#     accept() returns; our code never sees it.
#   - Reliability — every byte is accounted for via cumulative ACKs.
#     If an ACK doesn't arrive, the kernel retransmits automatically.
#     (Cumulative means one ACK can cover multiple segments at once,
#     e.g. "I've received everything up to byte 4381".)
#   - Ordering — if packets arrive out of order, the kernel buffers
#     them and only delivers contiguous data to recv().
#   - Segmentation — large sends are split into MSS-sized segments
#     (typically 1460 bytes on Ethernet, ~65 KB on loopback) and
#     reassembled transparently on the other side.
#   - Flow control — sliding window prevents a fast sender from
#     overwhelming a slow receiver.
#   - Congestion control — sending rate adapts to network conditions.
#   - Checksums — every segment's integrity is verified.
#   - Connection teardown — close() triggers the FIN/FIN-ACK sequence.
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
    ...
    # values (e.g. 128).
    num_of_clients = 2
    server_socket.listen(num_of_clients)
